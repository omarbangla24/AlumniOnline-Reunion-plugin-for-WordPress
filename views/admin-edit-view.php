<div class="wrap">
    ...
    <form method="post" action="?page=reunion-registrations" enctype="multipart/form-data">
        ...
        <table class="form-table">
            <tr><th scope="row"><label for="unique_id">Registration ID <span style="color: red;">*</span></label></th><td><input type="text" name="unique_id" value="<?php echo esc_attr($record->unique_id); ?>" class="regular-text" required><p class="description">You can change the registration ID here.</p></td></tr>
            ...
            <tr><th scope="row"><label for="reg-name">Name <span style="color: red;">*</span></label></th><td><input type="text" name="reg_name" value="<?php echo esc_attr($record->name); ?>" class="regular-text" required></td></tr>
            <tr><th scope="row"><label for="father_name">Father's Name <small style="color:#666;">(Optional)</small></label></th><td><input type="text" name="father_name" value="<?php echo esc_attr($record->father_name); ?>" class="regular-text"></td></tr>
            <tr><th scope="row"><label for="mother_name">Mother's Name <small style="color:#666;">(Optional)</small></label></th><td><input type="text" name="mother_name" value="<?php echo esc_attr($record->mother_name); ?>" class="regular-text"></td></tr>
            <tr><th scope="row"><label for="profession">Profession <small style="color:#666;">(Optional)</small></label></th><td><input type="text" name="profession" value="<?php echo esc_attr($record->profession); ?>" class="regular-text"></td></tr>
            <tr>
                <th scope="row"><label for="blood_group">Blood Group <small style="color:#666;">(Optional)</small></label></th>
                <td>
                    <select name="blood_group" class="regular-text">
            ...
            <tr><th scope="row"><label for="reg-batch">Batch <span style="color: red;">*</span></label></th><td><select name="reg_batch" required><?php $cy=date('Y');for($i=$cy;$i>=1997;$i--):?><option value="<?php echo $i;?>" <?php selected($record->batch, $i);?>><?php echo $i;?></option><?php endfor;?></select></td></tr>
            <tr><th scope="row"><label for="tshirt_size">T-Shirt Size <span style="color: red;">*</span></label></th><td><input type="text" name="tshirt_size" value="<?php echo esc_attr($record->tshirt_size); ?>" class="regular-text" required></td></tr>
            <tr><th scope="row"><label for="mobile-number">Mobile Number <span style="color: red;">*</span></label></th><td><input type="tel" name="mobile_number" value="<?php echo esc_attr($record->mobile_number); ?>" class="regular-text" required></td></tr>
            
            <tr><th scope="row"><label for="profile_picture_url">Profile Picture URL</label></th><td><input type="url" name="profile_picture_url" value="<?php echo esc_attr($record->profile_picture_url); ?>" class="regular-text"><p class="description">You can change the profile picture URL here.</p></td></tr>
            <tr><th scope="row"><label for="new_profile_picture">Upload New Profile Picture</label></th><td><input type="file" name="new_profile_picture" accept="image/*"><p class="description">Upload a new profile picture to replace the current one.</p></td></tr>
            
            <tr><th scope="row">Spouse Attending? <span style="color: red;">*</span></th><td><label><input type="radio" name="spouse_status" value="Yes" <?php checked($record->spouse_status, 'Yes'); ?> onclick="toggleVisibility('spouse-details', true)"> Yes</label> <label><input type="radio" name="spouse_status" value="No" <?php checked($record->spouse_status, 'No'); ?> onclick="toggleVisibility('spouse-details', false)"> No</label></td></tr>
            ...
            <tr><th scope="row">Payment Option <span style="color: red;">*</span></th><td><select name="payment_method" onchange="togglePaymentFields(this.value)" required><option value="bKash" <?php selected($record->payment_method,'bKash');?>>bKash</option><option value="Bank" <?php selected($record->payment_method,'Bank');?>>Bank Transfer</option></select></td></tr>
            ...
            <tr><th scope="row"><label for="total_fee">Total Fee <span style="color: red;">*</span></label></th><td><input type="number" step="0.01" name="total_fee" value="<?php echo esc_attr($record->total_fee); ?>" class="regular-text" required> BDT</td></tr>
            <tr><th scope="row"><label for="status">Status</label></th><td><select name="status" class="regular-text"><option value="Pending" <?php selected($record->status, 'Pending'); ?>>Pending</option><option value="Paid" <?php selected($record->status, 'Paid'); ?>>Paid</option><option value="Cancelled" <?php selected($record->status, 'Cancelled'); ?>>Cancelled</option></select></td></tr>
        </table>
